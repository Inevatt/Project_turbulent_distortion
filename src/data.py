...
носитель ядра при максимальном D/r0. При diffraction_fwhm_px=2.0
и D/r0 <= 5 это sigma <= 4.25 px, обрез scipy на 4*sigma даёт 17 px;
взято 16 — хвост за обрезом 0.02% веса, зато 160 = 128 + 2*16 удобнее
для FFT в D2. Функцией от D/r0 запас делать нельзя: размер вырезаемого
окна поехал бы вместе с силой турбулентности, и распределение позиций
кропа стало бы зависеть от D/r0.
"""
