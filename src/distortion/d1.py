...
class D1TipTilt(Degradation):
    ...
    и усреднение многих кадров D1 даёт ровно D0. Для гауссиан это
    тождество, а не приближение: свёртка гауссиан есть гауссиана,
    оси независимы. Деление задано в optics.sigma_pair.

    По форме PSF это всё ещё гаусс, то есть D1 — не «мгновенный кадр»,
    а усреднение по всему КРОМЕ наклона. Форма умирает на D2, и разница
    D1 против D2 при том же D/r0 остаётся чистой разницей формы, потому
    что ширина у них совпадает по построению. В этом весь смысл уровня:
    D0 -> D1 меняет только ширину, D1 -> D2 только форму.
    """
