...
36 ячеек матрицы. Изменение здесь обесценивает уже посчитанные ячейки,
...
def receptive_field(depth=3):
    """Условный локальный RF свёрточного пути (без GroupNorm), в пикселях.

    GroupNorm связывает все пространственные позиции через статистики,
    поэтому фактическая зависимость выхода — от всего входного кропа.
    Локальный расчёт ниже является верхней оценкой поддержки интерполяции.
    ...
    for _ in range(depth):
        r += j                         # два соседа на СТАРОЙ сетке
